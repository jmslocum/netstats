/*
   the netstats package is a Go interface to the /proc/net/dev
   file on the Linux file system. This file contains all I/O stats
   for all network cards on the unit. By obtaining DeviceStats at
   a set interval, the user can determine throughput for each
...
/*
   The number of compressed packets received by the device
   driver
*/
func (stats *DeviceStats) RxCompressed() uint64 {
...
/*
   The total number of compressed packets transmitted by the
   named device
*/
func (stats *DeviceStats) TxCompressed() uint64 {
	return stats.TxStats[7]
}
